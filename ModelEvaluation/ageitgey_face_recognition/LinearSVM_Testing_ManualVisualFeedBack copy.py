...
arcFaceNet= cv2.dnn.readNetFromONNX(sau.arcface_onnx_model_path)
...
if(no_of_testImages>0):
    for (i_img, testImage_path) in enumerate(testImage_paths):
        ...
        face_locations_1= face_recognition.face_locations(
            testImage,
            model=args["model"],
            number_of_times_to_upsample= int(args["upscale"])
        )

        aligned_faces, batch_landmark, img_face_list =cb.get_batch_crop_faces_from_diff_input_size([testImage])
        face_locations= batch_landmark[0]
        ...
        if(no_of_faces>0):
            ...
            face_encodings=[]
            for alfa in aligned_faces:
                face_blob= cv2.dnn.blobFromImage(alfa, 1, (112, 112))
                arcFaceNet.setInput(face_blob)
                face_encodings.append(arcFaceNet.forward()[0])
